...
-- 刪除現有的更新策略（舊名稱和新名稱都刪除）
DROP POLICY IF EXISTS "Users can update logs" ON daily_logs;
DROP POLICY IF EXISTS "Anyone can update logs" ON daily_logs;
...
DROP POLICY IF EXISTS "Anyone can insert logs" ON daily_logs;
...
DROP POLICY IF EXISTS "Anyone can view logs" ON daily_logs;
